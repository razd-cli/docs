...
> Базовый `Razdfile.yml` содержит шаблонные задачи. Вам потребуется добавить необходимые инструменты в секцию `mise.tools` (например, `node: "22"`, `python: "3.11"`), заменить команды `echo` на реальные команды вашего проекта. Секция `tasks` основана на стандарте [Taskfile.yml](https://taskfile.dev/).
